...
var urlParser = {
  ...
  parse: function (url) {
    ...
    if (url.startsWith('useful:') && !url.startsWith('useful://app/')) {
      // convert shortened useful:// urls to full ones
      const urlChunks = url.split('?')[0].replace(/useful:(\/\/)?/g, '').split('/')
      const query = url.split('?')[1]
      return 'useful://app/pages/' + urlChunks[0] + (urlChunks[1] ? urlChunks.slice(1).join('/') : '/index.html') + (query ? '?' + query : '')
    }
    ...
    if (urlParser.isInternalURL(url)) {
      ...
      if (representedURL) {
        ...
        try {
          ...
          var urlObj = new URL(url)
          if (pageName) {
            return 'useful://' + pageName[1] + urlObj.search
          }
        } catch (e) {}
      }
    }
