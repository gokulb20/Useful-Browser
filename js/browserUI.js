...
// Branch Browser: Handle new-tab event with parent context for branch tracking
// Note: Branch creation is handled by branchEvents.js to avoid duplicates
webviews.bindEvent('new-tab', function (tabId, url, openInForeground, parentTabId) {
  // Get parent branch info to pass to branchEvents.js via tabData
  var parentTab = tabs.get(parentTabId || tabId)
  var parentBranchId = parentTab ? parentTab.branchId : null

  // Pass parentBranchId in tabData so branchEvents.js can create the branch
  // with the correct parent (not hardcoded ROOT)
  var newTab = tabs.add({
    url: url,
    private: tabs.get(tabId).private,
    parentBranchId: parentBranchId  // branchEvents.js will use this
  })

  // Don't create branch here - branchEvents.js handles it via tab-added event
